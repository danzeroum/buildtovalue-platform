...
import { classificationsForRef } from '../src/registry/store.js';
...
/** start → classificar(agentTask, ref flutuante 'agnt-aprova') → fim. */
function processWithAgent(): BpmnDiagram {
  ...
  d.edges.e2 = createEdge({ id: 'e2', sourceId: 'classificar', targetId: 'fim' });
  return d;
}
...
describe('AG-2.2 etapa 4 — ciclo COMPLETO com pin (fecha a etapa)', () => {
  ...
  beforeAll(async () => {
    ...
    // definição de processo com agentTask, inserida direto (o deploy/lint é testado à parte)
    await withTenant(migrator, tenant, (tx) => tx`
      INSERT INTO process_definitions (tenant_id, name, version, registry_ref, diagram, engine_version)
      VALUES (${tenant}, 'com-agente', 1, 'com-agente@1', ${tx.json(processWithAgent() as never)}, 'e')`);
  ...
  it('agentTask → CreateJob(agent) → substituição do pin → execução → trilha granular com ator', async () => {
    ...
    // 4) CONCLUSÃO pelo contrato: JobCompleted → o engine retoma e a instância completa.
